...
async function main() {
  ...
  for (i = 0; i < nets.nodes.length; i++) {
    try{
        tasks.add("Start node{} on "+nets.nodes[i].ip);
        await startCmd(nets.nodes[i]);
        tasks.complete()
      }catch(e){
          tasks.error(e)
      }
...
async function startCmd(node){
  ...
  ssh.exec('cd devnet;./cli/start.sh -f http://'+node.ip+':7545 -s http://'+node.ip+':8545 -r true -m true &', {silent:true})
  .start();
}
